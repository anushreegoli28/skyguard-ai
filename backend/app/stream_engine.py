...
class WeatherStreamEngine:
    ...
    def __init__(self):
        ...
        self.raw_dataset = fetch_noaa_ghcnh_data(
            days=30
        )
        ...
        self.anomaly_engine.train_isolation_forest(
            self.raw_dataset
        )
    ...
    def tick(self) -> Dict[str, Any]:
        """
        Advance the weather simulation by one timestep.
        """

        raw_obs = self.raw_dataset.iloc[
            self.current_index % self.total_records
        ].to_dict()
        ...
        # Apply active fault/event
        modified_obs = self.injector.apply_fault(
            raw_obs
        )

        # Anomaly engine
        anomaly_metrics = (
            self.anomaly_engine.process_observation(
                modified_obs
            )
        )
        ...
        health_metrics = (
            self.health_monitor.get_health_metrics()
        )
    ...
    def reset_station(self):
        ...
        print(
            "[STREAM ENGINE] Station reset "
            "to nominal operational baseline."
        )
